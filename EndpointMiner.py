#!/usr/bin/env python3
"""
deep_endpoint_finder.py (with MITM proxy capture mode)
Modes:
  --mode auto        : crawl starting domain (no auth)
  --mode manual      : use captured URLs file (one-per-line or HAR JSON)
  --mode proxy       : launch browser via selenium-wire (requires selenium-wire; unchanged)
  --mode proxy-mitm  : start mitmdump to capture requests; use FoxyProxy or browser proxy to 127.0.0.1:8080,
                      browse/login manually, then press ENTER in terminal to stop capture and scan captured URLs.

Install prerequisites:
  For general crawling: pip install --user requests beautifulsoup4
  For selenium proxy mode (optional): pip install --user selenium-wire selenium webdriver-manager
  For mitm proxy mode: pip install --user mitmproxy
Notes:
  - Only run scans against domains you own/are authorized to test.
  - This file contains both proxy capture options; pick one mode to use.
"""
